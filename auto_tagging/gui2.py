...
class App(tk.Tk):
    def __init__(self):
        ...
        ttk.Button(top, text="Copy", command=self.copy_tags).grid(row=0, column=7, padx=6)  # ✅ 복사 버튼
    ...
    def copy_tags(self):
        """리스트의 태그를 클립보드로 복사 (콤마 구분 / 티스토리·노션에 바로 붙여넣기 좋음)"""
        tags = [self.lst.get(i) for i in range(self.lst.size())]
        if not tags:
            messagebox.showinfo("알림", "복사할 태그가 없습니다. 먼저 실행을 눌러주세요.")
            return
        text = ", ".join(tags)           # 필요하면 "\n".join(tags) 로 변경 가능
        try:
            self.clipboard_clear()
            self.clipboard_append(text)
            self.update()  # 일부 OS에서 즉시 반영 위해
            self.write_log(f"클립보드로 복사: {text}")
            messagebox.showinfo("복사됨", "태그가 클립보드에 복사되었습니다.")
        except Exception as e:
            messagebox.showerror("오류", f"클립보드 복사 실패\n{e}")
            self.write_log(f"[오류] 복사 실패: {e}")
